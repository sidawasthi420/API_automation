...
public class VPSSendFaxStepDef {
    ...
    public VPSSendFaxStepDef(TestContext testContext){
    ...
    @Given("^User sets the baseURI and basePath for SendFax request$")
    public void userSetsTheBaseURIAndBasePathForSendFaxRequest() throws Exception{
    ...
    public void userSendsTheAPIRequestToSendFaxRequest() throws Exception {
    ...
    public void storeThePaymentReferenceNumberAndDisplayOnConsoleForSendFaxRequest() {
        ...
        String description = vps_main.getVPSAPIParameter(bodyObj,"PaymentRS..paymentResult.description");

        assertionService.assertThat("Verify description matches", description, CoreMatchers.containsString("Successful"));

        assertionService.assertThat("Verify paymentResult code matches", code, CoreMatchers.containsString("SUCCESS"));

        System.out.println("paymentReferenceNumber for SendFax Request :- " + paymentReferenceNumber);
    }
}
